...
pub fn noise_reduction(spectrum: &[Vec<Vec<f64>>]) -> Vec<Vec<Vec<f64>>> {
    ...
    let mut spectrum_average = vec![vec![vec![0.0f64; k]; n_bands]; l];
    for t in 0..l {
        for z in 0..n_bands {
            for bin in 0..k {
                spectrum_average[t][z][bin] = if z == 0 {
                    (spectrum[t][0][bin] + spectrum[t][1][bin]) / 2.0
                } else if z == n_bands - 1 {
                    (spectrum[t][n_bands - 1][bin] + spectrum[t][n_bands - 2][bin]) / 2.0
                } else {
                    (spectrum[t][z - 1][bin] + spectrum[t][z][bin] + spectrum[t][z + 1][bin]) / 3.0
                };
            }
        }
    }

    let mut phi_e = vec![vec![vec![0.0f64; k]; n_bands]; l];
    for t in 0..l {
        ...
        let s: Vec<f64> = (0..k)
            .map(|bin| {
                (0..n_bands)
                    .map(|z| spectrum_average[t][z][bin])
                    .sum::<f64>()
            })
            .collect();
        ...
        for z in 0..n_bands {
            for bin in 0..k {
                phi_e[t][z][bin] = spectrum_average[t][z][bin] * noise_suppression_weighting[bin];
            }
        }
    }
